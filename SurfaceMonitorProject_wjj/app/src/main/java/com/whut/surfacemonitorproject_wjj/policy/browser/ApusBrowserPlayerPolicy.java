...
/**
 * Created by wujiajun on 2017/4/20.
 */

public class ApusBrowserPlayerPolicy extends PlayerPolicy {

    private final String TAG = "ApusBrowserPlayerPolicy";

    private final static int UC_BROWSER_VIDEO_RESOLUTION_THREDHOLD = 600;
    private final static int UC_BROWSER_VIDEO_ADVERTISE_TIME = 90 * 1000;
    private final static int UC_BROWSER_VIDEO_SHOWTOAST_TIME = 300 * 1000;
    private final static int UC_BROWSER_TOAST_DISP_PERIOD = 100;
    private final static int UC_WIFI_THREDHOLD = 50;
    ...
    public ApusBrowserPlayerPolicy (Context context, SurfaceEvent event) {
    ...
    public String invokSurfaceDump() {
        ...
        if (out != null) {
            ...
            if (mIsAllowToast && minRes <= UC_BROWSER_VIDEO_RESOLUTION_THREDHOLD) {
                // video height is smaller than 600 in ApusBrowserPlayerPolicy, will toast
                if (!mEventHandler.hasMessages(MSG_SURFACE_LOWRES) && score >= UC_WIFI_THREDHOLD
                        && Utils.getScreenDirection(mContext) == Configuration.ORIENTATION_LANDSCAPE) {
                    mEventHandler.sendEmptyMessageDelayed(MSG_SURFACE_LOWRES, UC_BROWSER_VIDEO_ADVERTISE_TIME);
                    if(!mEventHandler.hasMessages(MSG_TIPS_OVER_3MIN)){
                        mEventHandler.sendEmptyMessageDelayed(MSG_TIPS_OVER_3MIN, UC_BROWSER_VIDEO_SHOWTOAST_TIME);
                    }
                }
            } else {
                mEventHandler.removeMessages(MSG_SURFACE_LOWRES);
