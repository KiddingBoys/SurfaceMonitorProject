...
public class YoukuPolicy extends PlayerPolicy {
	private final static int YOUKU_VIDEO_RESOLUTION_THREDHOLD = 600;
	private final static int YOUKU_VIDEO_ADVERTISE_TIME = 90 * 1000;
	private final static int YOUKU_TOAST_DISP_PERIOD = 100;
	private final static int YOUKU_WIFI_THREDHOLD = 50;
	...
	public YoukuPolicy(Context context, SurfaceEvent event) {
	...
	private int playerStatus(long totalTime, long cpuTime) {
		...
		if (cpuRatio < 0.03) return NO_PLAY;
	    return PLAY;
	}
	...
	public String invokSurfaceDump() {
		...
		if (mIsAllowToast && out != null) {
			...
			if (minRes <= YOUKU_VIDEO_RESOLUTION_THREDHOLD) {
				// video height is smaller than 600 in Letv, will toast
				if (!mEventHandler.hasMessages(MSG_SURFACE_LOWRES) && score >= YOUKU_WIFI_THREDHOLD) {
				    mEventHandler.sendEmptyMessageDelayed(MSG_SURFACE_LOWRES, YOUKU_VIDEO_ADVERTISE_TIME);
				}
			} else {
			    mEventHandler.removeMessages(MSG_SURFACE_LOWRES);
			}
