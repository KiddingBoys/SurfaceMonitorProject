...
public class LetvClientPolicy extends PlayerPolicy {

	private final static int LETV_VIDEO_RESOLUTION_THREDHOLD = 600;
	private final static int LETV_VIDEO_ADVERTISE_TIME = 90 * 1000;
	private final static int LETV_VIDEO_SHOWTOAST_TIME = 30000 * 1000;
	private final static int LETV_TOAST_DISP_PERIOD = 100;
	private final static int LETV_WIFI_THREDHOLD = 45;
	...
	public LetvClientPolicy(Context context, SurfaceEvent event) {
	...
	public String invokSurfaceDump() {
		...
		if (out != null) {
			...
			if (mIsAllowToast && minRes <= LETV_VIDEO_RESOLUTION_THREDHOLD) {
				// video height is smaller than 600 in Letv, will toast
				if (!mEventHandler.hasMessages(MSG_SURFACE_LOWRES) && score >= LETV_WIFI_THREDHOLD
						&& Utils.getScreenDirection(mContext) == Configuration.ORIENTATION_LANDSCAPE) {
				    mEventHandler.sendEmptyMessageDelayed(MSG_SURFACE_LOWRES, LETV_VIDEO_ADVERTISE_TIME);
				    if(!mEventHandler.hasMessages(MSG_TIPS_OVER_3MIN)){
				    	mEventHandler.sendEmptyMessageDelayed(MSG_TIPS_OVER_3MIN, LETV_VIDEO_SHOWTOAST_TIME);
				    }
				}
			} else {
			    mEventHandler.removeMessages(MSG_SURFACE_LOWRES);
		...
		if(Utils.getScreenDirection(mContext) != Configuration.ORIENTATION_LANDSCAPE){
			resetAll();
		}
